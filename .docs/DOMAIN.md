...
The **one** thing that lifts an opening is the „Měnit tip" entitlement („Počkejte si na
sestavy", or the premium toggle): that booster buys **both ends** of the window, so its
owner tips a waiting match immediately and then keeps tipping until the deadline. A waiting
card therefore carries the booster's paywall („Už to nevydržíte a nechcete čekat?"), and
`TipChangeUnlock` offers it whenever buying would lift the opening — even when the deadline
itself would not move. The **userless** window keeps its opening, so nothing here changes
