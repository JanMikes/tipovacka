...
- **premium ON US** (`premiumSponsoredAt`, admin only): the same premium, billed to nobody.
  A GLOBAL competition could always run premium at no user's expense because an admin owns
  it; this is that for a **private** partička, and it is the only way to have one. Granting
  it turns premium on together with all three feature toggles (a gift whose features are off
  is a gift of nothing) and forgives any existing *uncovered* charge — no credits move, an
  uncovered row was never debited. Charged rows stay charged. Nothing is charged on join or
  on re-enable, so reconciliation at start finds zero uncovered rows and simply confirms:
  **`ReconcilePremiumCompetitions` knows nothing about sponsorship.** Withdrawing keeps the
  premium the group has and only resumes billing for the *next* joiner. Granted from
  `/admin/souteze` („Prémium na nás") or `app:competition:sponsor-premium … --by=<admin>`;
  the handler re-checks ROLE_ADMIN because the console has no firewall. Global competitions
  are refused (`CompetitionIsGlobal::premiumIsAlreadyOnUs`).
...
do not) — the **winner after extra time / penalties** (`overtime_exact`; the stored pair is
NOT a score but the regular-time draw plus ONE goal for the winner, 2:2 → 3:2 / 2:3, whatever
happened in the prolongation or the shootout — `Value\OvertimeOutcome` is its one home; a tip
is „remíza X:X a vyhraje A", offered as a radio ONLY when the user tips a draw, the rule is
enabled AND the zdroj plays extra time; the rule scores the WINNER, the draw itself is scored
by the base rules), and scorer hit (points × number of correctly guessed scorers).
**Whether a draw can go on at all is a property of the zdroj zápasů** (`MatchSource.hasOvertime`,
off by default): a league keeps every draw as the final result and nobody is asked anything;
a cup / play-off zdroj asks the organizer ONE question on a drawn result — „remíza je konečný
výsledek" (default) / „vyhrál A" / „vyhrál B" — and never a second score (2026-09-03).
Turning the switch off later gates NEW entry only: winners already recorded keep displaying
(always as the team's name, never as the pair) and scoring. All optional rules default to
disabled.
A soutěž may enable `overtime_exact` over zdroje that never play extra time — the „Maxi"
preset does — where it can never score. The rule is **never hidden or auto-disabled** for
that (a soutěž can span zdroje with different flags, and a flag can be switched on later);
both rule surfaces say so instead, in three states resolved by `Enum\OvertimeCoverage`
(`None` / `Partial` / `All`) — the ONE home of that copy, as `RULE_COPY` is of the rule's
name (2026-09-03).
...
at creation; it drives period structure and copy. Whether a draw goes on to extra time /
penalties is the zdroj's own switch (`hasOvertime`, see §Scoring), not the sport's. Model is
...
| 2026-08-09 | **„Prémium na nás": an admin can grant a PRIVATE group premium at our expense** (`Competition.premiumSponsoredAt`, nullable — NULL = billed to the organizer as before). The whole mechanism is „create no charge rows": `ChargePremiumOnMemberJoinedHandler` and `EnablePremiumHandler` return early, so the organizer's wallet never matters and the start-of-competition reconciliation finds nothing uncovered and confirms — `ReconcilePremiumCompetitions` is untouched and knows nothing about sponsorship. Granting also switches monetization to premium and turns all three feature toggles on, and forgives pre-existing *uncovered* charges (marked Refunded, no credit movement — they were never debited; already-Charged rows are left alone). Withdrawing keeps the premium the group has and only resumes billing for the next joiner. Two entry points, one command: the „Prémium na nás" button on `/admin/souteze` and `app:competition:sponsor-premium <id> --by=<admin e-mail>`; ROLE_ADMIN is re-checked in the HANDLER because the console has no firewall. Global competitions are refused | an admin could always run a GLOBAL competition on premium without any user paying, because an admin owns it — a partička had no equivalent, so „we want to give this group premium" was only expressible by topping up a private person's wallet. That workaround is a running balance, not a decision: it has to stay ahead of every join, and the first one it fails to cover silently downgrades the whole soutěž at its first kickoff. Sponsorship makes the intent a stored fact instead, which is why the flag suppresses the CHARGE rather than forgiving it afterwards — a forgiven charge still has to be noticed, and an uncovered one downgrades the group exactly when it starts playing |
| 2026-08-13 | **A soutěž that never got per-match uzávěrky can be migrated to „5 h před výkopem" in one sweep.** `app:tip-opening:bulk-set --deadline-before-kickoff=300 --only-missing-deadline` writes row 1 of the deadline resolver for every match still on the competition-wide default, and touches nothing an organizer already decided. Run on production this day against Omnicado tipovačka (12), SK Brušperk (16) and Premier League pro Fantasy Magory (380) — 408 zápasů. The DEFAULT rule is unchanged: a new soutěž still locks at its start. | „Tipy se zamykají startem soutěže" fits a partička playing one turnaj and breaks a league that runs ten months — one kickoff in August froze every tip until May. The pre-existing escape hatch (`--deadline-own-kickoff`) took tips right up to the whistle and overwrote stored uzávěrky, so it could be neither a head start for the players nor a safe migration. Note the consequence when the soutěž has already started: its remaining zápasy REOPEN for tipping, after members may already have seen each other's tips — deliberate here, worth weighing per run |
| 2026-09-03 | **A regular-time draw is a complete final result; overtime / penalties are an explicit per-match opt-in on the result form, never implied by the draw.** `SetFinalScoreFormData.decidedInOvertime` gates the after-overtime pair: unticked ⇒ the two overtime inputs are dropped before validation and dispatch (the Stimulus controller also hides + disables them, and disables the box itself while the score is not a draw, so a ticked-but-invisible opt-in can never be submitted); ticked ⇒ both sides required and the existing invariants apply (draw in regular time, not a draw, ≥ regular). No schema change: „no overtime" stays `overtimeHomeScore = overtimeAwayScore = NULL`; correcting a match that has one pre-ticks the box. The draw-rejection copy now says how to get out („nechte prodloužení vypnuté") | The organizer of Omnicado tipovačka could not save Panathinaikos – Hradec Králové 2:2 (a Conference League qualifier FIRST leg, no extra time): the form revealed „Po prodloužení / nájezdech – Konečný stav" on its own the moment 2:2 was typed, with no „(nepovinné)" (the periods block has one), so they typed the final score there again and hit „Skóre po prodloužení nemůže být remíza". Production had never stored an overtime score in 316 finished matches and both private-source draws had been saved with the block left empty — the copy, not the rule, was the bug. A yes/no question the organizer can answer („went to extra time?") beats an input row they must know to leave blank; tournaments without extra time end in draws and the app must say so rather than suggest a second score |
| 2026-09-03 | **After a regular-time draw the app records WHO WON, never a second score — and only where the zdroj plays extra time at all.** (1) The after-overtime pair is fixed to the draw plus ONE goal for the winner (2:2 → 3:2 / 2:3; `Value\OvertimeOutcome`, enforced by both entities), so the organizer answers one radio — „remíza je konečný výsledek" / „vyhrál A" / „vyhrál B" — and a player's tip is „remíza X:X a vyhraje A" (a radio, no inputs). `overtime_exact` scores the winner only; the draw is the base rules' job. A partial UI (batch / on-behalf) carries the pick over to a corrected draw (1:1 → 2:2 keeps „vyhraje A"). (2) `MatchSource.hasOvertime` (default off; set on the zdroj create/edit forms, the admin curated form and the wizard's from-scratch path) gates it all: off ⇒ the result form builds no question, the guess form shows no radio, and both entities refuse a NEW overtime pair; winners recorded earlier keep displaying and scoring (the switch gates entry, it never deletes data), and a score-only feed keeps a hand-entered winner across polls. This supersedes the same-day „opt-in checkbox + two inputs" row above | The product owner's words: „when tie is 2:2 and either extra time one team finished or penalties or anything, then it is 3:2 or 2:3 — we should not care about the penalties exactly, it is just who won" and „we must still support ties without extra time enabled on the tournament and with extra time enabled — both scenarios". A real after-extra-time score (2:2 → 4:2, or 1:1 → 4:3 on penalties) has no honest single-pair encoding and nobody tips one anyway; the winner is the only fact a tipovačka needs. Making „does a draw go on?" a fact about the zdroj — not the sport (a football league and a football cup differ) and not the soutěž (the same zápas is one fact in every soutěž) — is what lets a plain league draw save with no question asked, which is exactly where the Omnicado organizer got stuck |
| 2026-09-03 | **The UEFA feed now fills the overtime winner in itself — from THIS match's own extra time or shootout, and from nothing else.** `UefaMatchDataProvider` reads `score.total` (this match including extra time), falling back to `score.penalty` (singular, only on the leg where the shootout was taken), and only for a FINISHED regular-time draw; the pair it emits is `OvertimeOutcome`'s draw-plus-one, never UEFA's real after-extra-time score (Pafos–Dinamo City 4:2 is stored as 3:2). Deliberately unread: `score.aggregate` and `winner.aggregate`, which describe the TIE and are repeated on the first leg, and `winner.match.reason`, which only ever restates whether the 90 minutes were a draw. Ambiguous or missing ⇒ null = unknown, exactly as before. Where a snapshot carries a winner but the zdroj's `hasOvertime` is off, `FeedSynchronizer` drops it in both the write and the unchanged-check and reports it once as a WARNING (`FeedSyncResult::$overtimeNotPlayed`) | the convention was locked the same morning but the adapter was left reporting the 90-minute score „until a real shootout payload confirms the field shapes" — so the shapes were read: all 824 rows of the 2026/27 UCL+UEL+UECL seasons, 21 real shootouts and 11 real extra times (evidence table in [`.docs/MATCH_DATA_FEEDS.md`](MATCH_DATA_FEEDS.md) §Adapter 2). The doc had guessed `score.penalties`; the field is `score.penalty`. The trap that decided the rule is Egnatia–Celje 2048724, a plain 3:3 FIRST leg whose row already announces `WIN_ON_PENALTIES / Celje` for a shootout taken five days later in Slovenia — a tie-level fact that would have invented a winner for a match that drew. The dropped-winner warning is the other half: UEFA's competitions serve knockout ties whatever the zdroj's checkbox says, and an entity that refuses the pair would otherwise throw on all 288 daily polls |
| 2026-09-03 | **A scoring rule that cannot score in this soutěž is annotated, never hidden and never auto-disabled.** `overtime_exact` only scores where the zápas's zdroj plays extra time (`MatchSource.hasOvertime`), so a soutěž over zdroje that all have it off can turn the rule on — the „Maxi" preset does — and get silence. Both rule surfaces now carry a three-state hint next to the rule, resolved by `Enum\OvertimeCoverage` (`None` / `Partial` / `All`), which is the ONE home of the copy exactly as `RulePresetProvider::RULE_COPY` is of the rule's name. The post-creation screen takes it from the query layer (`CompetitionRuleConfigurationResult.overtimeCoverage`, one COUNT/SUM over the soutěž's `CompetitionSource` layers), the wizard's „Pravidla" step derives it from the košík composed in step 1 — where a soutěž of nothing but its own matches is pointed at the step holding the switch rather than at a zdroj settings page that does not exist yet | Hiding or disabling the rule would be wrong in both directions: a soutěž may span several zdroje with different flags (the rule really does score, just not everywhere — hence the softer middle state), and a flag can be switched on later, at which point a rule silently removed at create time would never come back. Saying the truth out loud costs one line and leaves the organizer in charge |
