...
| vylepšení / boost | `BoostPurchase` (`BoostType`) | A per-competition boost a player buys in a `boosts` competition: Rozložení tipů / Konkrétní tipy / Měnit tip |
...
- **premium** („Zaplatím za celou skupinu"): manager pays 10 credits per player, charged at
  ...
  Premium managers toggle features for everyone: Rozložení tipů, Konkrétní tipy, Měnit tip
  (+ own change-deadline offset).
- **boosts** („Nechám příspěvek na jednotlivcích"): each player may buy per-competition
  boosts. Prices (constants in one config class): Rozložení tipů ostatních 10, Konkrétní tipy
  kolegů 20 (includes Rozložení tipů — superset, not prerequisite purchase), Měnit tip během
...
per-match override allowed (never after kickoff). With the „Měnit tip" entitlement: tips changeable
