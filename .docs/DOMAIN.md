...
(points × number of correctly guessed scorers). On the organizer's result form the
after-overtime pair is an explicit **per-match opt-in** („Zápas se rozhodl až v prodloužení
nebo na penalty"): a drawn regular-time result with the box unticked IS the complete final
result — leagues, groups and first legs of two-legged ties end in draws — and the overtime
inputs are then ignored, never validated (2026-09-03). All optional rules default to disabled.
