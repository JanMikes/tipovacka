...
too — and enforced in every guess-writing handler, not only in the UI. The opening is
**not** per viewer: no entitlement opens a window early, and it must fall strictly before
the match's effective deadline (rejected at write time, otherwise the match could never be
tipped).
