...
| `competition_detail` | `/souteze/{id}` | `portal/competition/detail.html.twig` — **a playing surface** since item 08: header (back link, eyebrow „zdroj · kolo", name + Live/Ukončeno/Tipy-uzamčeny pills, role badges) + a 5-item action bar (**Pravidla** — *no voter, every viewer* · **Nastavení** `competition_edit` · **Pozvat** `competition_manage_join_mechanics` · **Tipovat za členy** `competition_manage_members and not isGlobal` · **Uzamknout/Odemknout tipy** `competition_edit`), then **ONE column** (item 19 dissolved the aside): **popis soutěže** (cap `Competition::DESCRIPTION_MAX_LENGTH` = 1000, escaped + `whitespace-pre-line`, renders **nothing** when empty) → **„Pozvat kamaráda"** → `/souteze/{id}/nastaveni#pozvanky`, same voter as „Pozvat" → the „Tipněte si všechny zápasy najednou" banner → **„Tabulka soutěže"** + the match list, **5 rows with „Načíst všechny zápasy (N)"** below → **Žebříček** (`#zebricek`, real rows, „Celý žebříček" → `/zebricek?soutez=`). **Item 35 deleted the trailing `Boost:Panel` card („Získej výhody", `#vylepseni`)** — every booster is now sold at the thing it unlocks, so the page has no shop section and no `#vylepseni` anchor at all. The match list renders `Match:MatchRow` — one design on every surface since item 21. A plain member sees **only „Pravidla"** in the action bar, and no „Pozvat kamaráda". **„Pravidla" opens a read-only `<dialog>`** (`_rules_modal.html.twig` + the generic `modal` controller) listing this competition's *enabled* scoring rules and their points, rendered by `_partials/competition_rules.html.twig` in its new `bare` mode — the same partial „Nastavení" renders as a card. Configuration only: no form, no inputs, nobody's tips. **First visit as a member of a `boosts` competition opens a `<dialog>` with the boost prices** (`_boost_intro_modal.html.twig` + `boost-intro` controller); dismissal is stamped on `Membership.boostIntroSeenAt` via `POST …/vylepseni/uvod/skryt`, and it is suppressed for a non-member, on premium/`none`, and on a fully-over competition (B6) |
...
| `competition_my_tips_batch` | `/souteze/{id}/moje-tipy` | `portal/competition/my_tips_batch.html.twig` — the batch tip table, and since item 35 the **competition-wide „Počkejte si na sestavy" paywall** (`<twig:Boost:Panel feature="tip_change">`, inline shape, no `sportMatch`): it names the soonest zápas the boost would hand back and its regained uzávěrka, and renders **nothing** when buying would change nothing |
...
`Boost/BoostPanel` (owned boosts render a **jump link** to what they unlocked, unowned a purchase CTA; both disappear once the competition is fully over — B6. **Three shapes since item 35**: `feature=null` is the old „Získej výhody" card — **no call site any more**; `feature="others"` the tips paywall; `feature="tip_change"` the „Počkejte si na sestavy" paywall on the locked tip form and on `/moje-tipy`, whose printed moment comes from a SECOND `EffectiveTipDeadlineResolver` wired with `TipChangeGrantedEntitlements` (`app.tip_deadline_resolver.tip_change_granted`, read through `Service/Competition/TipChangeUnlock`) so nothing re-derives „výkop mínus offset". A caller holding only an id passes `competitionId`. Round 2: panel rows carry marketing headlines („Jak tipují ostatní?" / „Přesné tipy soupeřů" / „Počkejte si na sestavy") while **`BoostType::label()` still names the confirm dialog, the prémium toggles, `/cenik` and the ledger** — the headline is a label above the canonical product, not a rename) · `Notification/Bell` · `Notification/Preferences` · `CreditBalance` ·
...
`competition_matches` · `wizard_matches` · `boost_intro` · `modal`.

`modal` (item 26) opens/closes a server-rendered `<dialog>` and **freezes the document while it is open**, releasing on the `close` event — measured: an open `<dialog>` does **not** stop the page behind it scrolling in Chrome, so every other dialog in this app still scrolls away underneath.
