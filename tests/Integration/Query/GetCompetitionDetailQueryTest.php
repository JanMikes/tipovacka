...
final class GetCompetitionDetailQueryTest extends IntegrationTestCase
{
    public function testOwnerSeesSecrets(): void
    {
        $result = $this->queryBus()->handle(new GetCompetitionDetail(
            competitionId: Uuid::fromString(AppFixtures::VERIFIED_COMPETITION_ID),
            viewerId: Uuid::fromString(AppFixtures::VERIFIED_USER_ID),
            viewerIsAdmin: false,
        ));

        self::assertSame(AppFixtures::VERIFIED_COMPETITION_PIN, $result->pin);
        self::assertSame(AppFixtures::VERIFIED_COMPETITION_LINK_TOKEN, $result->shareableLinkToken);
