...
final class SendCompetitionInvitationHandlerTest extends IntegrationTestCase
{
    public function testPersistsInvitationAndNormalisesEmail(): void
    {
        $envelope = $this->commandBus()->dispatch(new SendCompetitionInvitationCommand(
        ...
        $persisted = $em->find(CompetitionInvitation::class, $invitation->id);
    ...
    public function testDoesNotDuplicateMembershipWhenInvitingExistingMember(): void
    {
        ...
        $before = (int) $em->createQueryBuilder()
            ...
            ->where('m.competition = :gid')->setParameter('gid', Uuid::fromString(AppFixtures::VERIFIED_COMPETITION_ID))
            ->andWhere('m.leftAt IS NULL')
            ->getQuery()->getSingleScalarResult();

        // Owner is already an active member of their own competition — reinviting them
