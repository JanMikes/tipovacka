...
final class AcceptCompetitionInvitationHandlerTest extends IntegrationTestCase
{
    public function testAcceptCreatesMembershipAndMarksAccepted(): void
    {
        $user = $this->createVerifiedUser();

        $this->commandBus()->dispatch(new AcceptCompetitionInvitationCommand(
            userId: $user->id,
    ...
    {
        $em = $this->entityManager();
        // Admin is already a member of PUBLIC_COMPETITION (owner). Make them accept the invitation.
        $this->commandBus()->dispatch(new AcceptCompetitionInvitationCommand(
    ...
    public function testExpiredInvitationThrows(): void
    {
        ...
        try {
            ...
            self::assertInstanceOf(CompetitionInvitationExpired::class, $e->getPrevious());
