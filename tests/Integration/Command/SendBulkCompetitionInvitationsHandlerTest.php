...
final class SendBulkCompetitionInvitationsHandlerTest extends IntegrationTestCase
{
    public function testCreatesStubUserForUnknownEmailAndIssuesInvitation(): void
    {
        $envelope = $this->commandBus()->dispatch(new SendBulkCompetitionInvitationsCommand(
    ...
    {
        // Verified user is already a member of VERIFIED_COMPETITION (they own it).
        $envelope = $this->commandBus()->dispatch(new SendBulkCompetitionInvitationsCommand(
            inviterId: Uuid::fromString(AppFixtures::VERIFIED_USER_ID),
            competitionId: Uuid::fromString(AppFixtures::VERIFIED_COMPETITION_ID),
    ...
    {
        // PUBLIC_COMPETITION has PENDING_INVITATION_EMAIL as pending.
        $envelope = $this->commandBus()->dispatch(new SendBulkCompetitionInvitationsCommand(
